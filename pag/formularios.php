...
<html>
	<head>
        ...
        <script>
            ...
            function limpiar(){
                ...
                document.principal.idFormulario.value=""
    ...
    if (!Permiso("formularios.php")) {    //funcion que verifica si el usuario tiene permiso al formulario.
        ...
        <form name="principal" method="POST" action="estatusObras.php">
            ...
            <?php
                echo "<input type='hidden' name='idFormulario' value='$idFormulario'>    <!-- es el estatus de la tabla tblformularios -->"
			...
			<tr>
                ...
                <td width="80%">
                    ...
                    if ($idFormulario!=""){
                        conectar();
                        $result = mysqli_query($_SESSION["Conexion"], "select  DesFormulario from tblformularios where idFormulario=".$idFormulario);
                        if (mysqli_num_rows($result)>0){
                            $row = mysqli_fetch_array($result);
                            echo "<input type='text' name='nombre' value='".$row['DesFormulario']."'>";
                        }
        ...
        <?php
            ...
            if(isset($_POST["accion"])){   // validamos que la variable este definida, para que no truene la primera vez que entra
				...
				if ($accion=="Actualizar"){       
					...
					if ($nombre==""){
						...
						if ($nombre!="" && $idFormulario!="") {   //QUIERE DECIR QUE SE JALO UNO QUE ESTA DESPLEGADO
							    $nombreOri=$nombre;
                                $result2 = mysqli_query($_SESSION["Conexion"], "update tblformularios set DesFormulario='$nombre' where idFormulario='$idFormulario'");
                                Mensaje("SE HAN ACTUALIZADO LOS DATOS !!!");
                        }else{
							$result = mysqli_query($_SESSION["Conexion"], "select * from tblformularios  where DesFormulario='$nombre'");
                            if (mysqli_num_rows($result)>0) {
                                Mensaje("Ya existe un registro   !!!");
                            }
							else{
                                Mensaje("Lo damos de alta !!!");
                                $result2 = mysqli_query($_SESSION["Conexion"],"insert into tblformularios(DesFormulario) values('$nombre')");   
                            }
                            mysqli_free_result($result);	  
                        }	
                ...
                if ($accion=="Eliminar"){
					...
					if ($nombre==""){
                        ...
                        // procedemos hacer la actualización, puede ser alta o modificación
						if ($nombre!="" && $idFormulario!="") {   //QUIERE DECIR QUE SE JALO UNO QUE ESTA DESPLEGADO
                                $result2 = mysqli_query($_SESSION["Conexion"], "delete from tblformularios where idFormulario='$idFormulario'");
            ...
            conectar();
            $result = mysqli_query($_SESSION["Conexion"], "select idFormulario, DesFormulario from tblformularios order by idFormulario ");
            if (mysqli_num_rows($result)>0){
                $titulos=array("No.","Nombre del formulario");
                 CreaTabla("Estatus de las Obras de Arte registrados en la Base de Datos",$result,$titulos,$inicio,"formularios.php","inicio",$RegistrosXPagina,false);	
                //EscribeBitacora(72, "comentario");			
            }
            else{
                Mensaje("No existen Formularios de Arte dados de alta en la Base de Datos !!!");    
            }	
            mysqli_free_result($result);
            Desconectar(); 
