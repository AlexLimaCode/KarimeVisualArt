...
<html>
	<head>
        ...
        <script>
            ...
            function limpiar(){
                ...
                document.principal.CveTipo.value=""
    ...
    if (!Permiso("tiposObras.php")) {    //funcion que verifica si el usuario tiene permiso al formulario.
        ...
        <?php
        $CveTipo = "";
        if(isset($_POST["CveTipo"])){
            $CveTipo = trim($_POST["CveTipo"]); 
            if ($CveTipo == ""){
                if(isset($_GET["CveTipo"])){
                    $CveTipo = $_GET["CveTipo"];
                    if ($CveTipo == ""){
                        $CveTipo = "";
                    }
                }
        ...
        <form name="principal" method="POST" action="tiposObras.php">
            ...
            <?php
                echo "<input type='hidden' name='CveTipo' value='$CveTipo'>    <!-- es el estatus de la tabla tblestatus -->"
			...
			<tr>
                ...
                <td width="80%">
                    ...
                    if ($CveTipo!=""){
                        conectar();
                        $result = mysqli_query($_SESSION["Conexion"], "select  NomTipo from tbltiposobras where CveTipo=".$CveTipo);
                        if (mysqli_num_rows($result)>0){
                            $row = mysqli_fetch_array($result);
                            echo "<input type='text' name='nombre' value='".$row['NomTipo']."'>";
                        }
        ...
        <?php
            ...
            if(isset($_POST["accion"])){   // validamos que la variable este definida, para que no truene la primera vez que entra
				...
				if ($accion=="Actualizar"){       
					...
					if ($nombre==""){
						...
						if ($nombre!="" && $CveTipo!="") {   //QUIERE DECIR QUE SE JALO UNO QUE ESTA DESPLEGADO
							    $nombreOri=$nombre;
                                $result2 = mysqli_query($_SESSION["Conexion"], "update tbltiposobras set NomTipo='$nombre' where CveTipo='$CveTipo'");
                                Mensaje("SE HAN ACTUALIZADO LOS DATOS !!!");
                        }else{
							$result = mysqli_query($_SESSION["Conexion"], "select * from tbltiposobras  where NomTipo='$nombre'");
                            if (mysqli_num_rows($result)>0) {
                                Mensaje("Ya existe un registro   !!!");
                            }
							else{
                                Mensaje("Lo damos de alta !!!");
                                $result2 = mysqli_query($_SESSION["Conexion"],"insert into tbltiposobras(NomTipo) values('$nombre')");   
                            }
                            mysqli_free_result($result);	  
                        }	
                ...
                if ($accion=="Eliminar"){
					...
					if ($nombre==""){
                        ...
                        // procedemos hacer la actualización, puede ser alta o modificación
						if ($nombre!="" && $CveTipo!="") {   //QUIERE DECIR QUE SE JALO UNO QUE ESTA DESPLEGADO
                                $result2 = mysqli_query($_SESSION["Conexion"], "delete from tbltiposobras where CveTipo='$CveTipo'");
            ...
            conectar();
            $result = mysqli_query($_SESSION["Conexion"], "select CveTipo, NomTipo from tbltiposobras order by CveTipo ");
            if (mysqli_num_rows($result)>0){
                $titulos=array("No.","Nombre del Tipo");
                 CreaTabla("Tipos de las Obras de Arte registrados en la Base de Datos",$result,$titulos,$inicio,"tiposObras.php","inicio",$RegistrosXPagina,false);	
