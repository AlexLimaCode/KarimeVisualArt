...
<html>
	<head>
        ...
        <script>
            ...
            function limpiar(){
                ...
                document.principal.IdEstatus.value=""
    ...
    if (!Permiso("tiposObras.php")) {    //funcion que verifica si el usuario tiene permiso al formulario.
        ...
        if(isset($_POST["IdEstatus"])){
            $IdEstatus = trim($_POST["IdEstatus"]); 
            if ($IdEstatus == ""){
                if(isset($_GET["IdEstatus"])){
        ...
        ?>
        <form name="principal" method="POST" action="tiposObras.php">
            <input type="hidden" name="accion" value="">        <!-- es la accion que hara el usuario al presionar un boton -->
            <?php
                echo "<input type='hidden' name='IdEstatus' value='$IdEstatus'>    <!-- es el estatus de la tabla tblestatus -->"
                
            ?>
            <table width="100%">  <!-- tabla para poner el titulo  -->
            <tr>
                <td width="100%"> > > Tipos de las Obras de Arte < < </td> 
            </tr> 
            </table>

            <table width="100%">  <!-- tabla para poner los campos del Formulario  -->
			<tr>
                <td width="20%">Nombre del Tipo de la Obra: </td>
                <td width="80%">
                    <?php
                    if ($IdEstatus!=""){
                        conectar();
                        $result = mysqli_query($_SESSION["Conexion"], "select  DesEstatus from tblestatusrecibo where IdEstatus=".$IdEstatus);
                        if (mysqli_num_rows($result)>0){
                            $row = mysqli_fetch_array($result);
                            echo "<input type='text' name='nombre' value='".$row['DesEstatus']."'>";
                        }
        ...
        <?php
            ...
            if(isset($_POST["accion"])){   // validamos que la variable este definida, para que no truene la primera vez que entra
				...
				if ($accion=="Actualizar"){       
					...
					if ($nombre==""){
						...
						if ($nombre!="" && $IdEstatus!="") {   //QUIERE DECIR QUE SE JALO UNO QUE ESTA DESPLEGADO
							    $nombreOri=$nombre;
                                $result2 = mysqli_query($_SESSION["Conexion"], "update tblestatusrecibo set DesEstatus='$nombre' where IdEstatus='$IdEstatus'");
                                Mensaje("SE HAN ACTUALIZADO LOS DATOS !!!");
                        }else{
							$result = mysqli_query($_SESSION["Conexion"], "select * from tblestatusrecibo  where DesEstatus='$nombre'");
                            if (mysqli_num_rows($result)>0) {
                                Mensaje("Ya existe un registro   !!!");
                            }
							else{
                                Mensaje("Lo damos de alta !!!");
                                $result2 = mysqli_query($_SESSION["Conexion"],"insert into tblestatusrecibo(DesEstatus) values('$nombre')");   
                            }
                            mysqli_free_result($result);	  
                        }	
                ...
                if ($accion=="Eliminar"){
					...
					if ($nombre==""){
                        ...
                        // procedemos hacer la actualización, puede ser alta o modificación
						if ($nombre!="" && $IdEstatus!="") {   //QUIERE DECIR QUE SE JALO UNO QUE ESTA DESPLEGADO
                                $result2 = mysqli_query($_SESSION["Conexion"], "delete from tblestatusrecibo where IdEstatus='$IdEstatus'");
            ...
            conectar();
            $result = mysqli_query($_SESSION["Conexion"], "select IdEstatus, DesEstatus from tblestatusrecibo order by IdEstatus");
            if (mysqli_num_rows($result)>0){
                $titulos=array("No.","Nombre del Tipo");
                 CreaTabla("Estatus de los Recibos en la Base de Datos",$result,$titulos,$inicio,"estatusRecibos.php","inicio",$RegistrosXPagina,false);	
                //EscribeBitacora(72, "comentario");			
            }
            else{
                Mensaje("No existen Estatus de Recibos dados de alta en la Base de Datos !!!");    
            }	
            mysqli_free_result($result);
            Desconectar(); 
